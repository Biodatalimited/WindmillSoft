/**
***	WPAO_DDE.c		ATK-PANELS - Toolkit Module
***				DDE Support Fns for
...
#define  SZDDE_SERVICENAME	"AnalogOut"
...
char szDdeHelp [] = "WINDMILL DISPLAY AND CONTROL PANEL - DDE HELP\r\n\r\n"\
	...
	"  'Load <setup filename>'- Loads a new AnalogOut setup file\r\n"\
	"  'Move <x> <y> <width> <height>'- Repositions and resizes AnalogOut\r\n"\
...
**/
extern	PANELINFO	panAnalogOut;	/* General application Information */
...
***		  szServiceName : The DDE Service Name e.g 'AnalogOut'
...
***	WPAO_DEF.h header because we would then need to include 'DDEML.h'
***	in all modules.
**/
...
short DdeStartConversation ( DWORD idInst )
{
	...
	if ( ++panAnalogOut.nNumDDELinks == 1 ) {
		...
		***	topics : Data, Range, Units and Error
		***
...
short DdeTerminateConversation ( DWORD idInst )
{
	...
	***	conversations.
	**/
	if ( --panAnalogOut.nNumDDELinks == 0 ) {
		...
		**/
		if ( nTopicId == TOPIC_ID_DATA ) {
			...
			**/
			...
			FORWARD_WM_READCHAN ( ddeInfo.hwndAppl,
						nItemId, SendMessage );
		}
		...
		if ( GetExecuteId ( hData, &nExecuteId, szParams ) ) {
			...
			case EXEC_ID_LOADSETUP:
				...
				**/
				...
				/**	Prepare application for new
				***	setup i.e scrollbars, focus channel
				***	etc.
				**/
...
VOID PTKAdviseDDE ( short nItem )
{
	...
	**/
	...
	/**	Get  string handles for the data and alarm topics
	**/
...
HDDEDATA DdeGetSysItemData ( short nSysItemId )
{
	...
	**/
	...
	case SYSITEM_ID_FORMATS :
		...
		/**	Only a single clipboard format supported
		**/
	...
	case SYSITEM_ID_STATUS :
		...
		lstrcpy ( lpszDdeData, (LPSTR) SZSTATUS_MESSAGE	);
		break;
...
static short ExtractPokeData ( HDDEDATA hData, short nItemId )
{
	...
	**/
	if ( nItemId == conninfo.nNumConnected ) {
		...
		for ( ulDataIndex = 0L; ulDataIndex < ulDataLength;
			...
			**/
			...
			if ( ( szChanValue [ nValueIndex ] == '\t' ) ||
				...
				**/
				lstrcpy ( conninfo.lpodOutputData
					[ nChannel ].szNewValue ,
						 (LPSTR) szChanValue );
		...
		***	WPAO_MSS module ready to be processed
		...
		**/
		lstrcpy ( conninfo.lpodOutputData [ nItemId ].szNewValue,
						   (LPSTR) szChanValue );

		/**	Flag the data as being new and
		***	requiring attention.
		**/
		conninfo.lpodOutputData [ nItemId ].bWriteData = TRUE;
