...
extern RECT rectSymbols;    /* Area occupied by exclaimation symbols	    */
			...
			if ( conninfo.lpodOutputData != NULL ) {
				...
				**/
				for ( nConnection = 0;
					...
					conninfo.lpodOutputData
					  [ nConnection ].nWriteError = OK;


					conninfo.lpodOutputData
					  [ nConnection ].szNewValue [ 0 ]
								      = '\0';
				}
...
short ChannelNumberToTextLine ( short nChannelNumber ) 
{
	...
	return ( nChannelNumber - panAnalogOut.nFirstChanVisible );
}
...
short TextLineToChannelNumber ( short nTextLine ) 
{
	...
	return ( nTextLine + panAnalogOut.nFirstChanVisible );
}
...
short PixelPositionToChannelNumber ( WORD wYPos )
{
	...
	**/
	...
	nTextLine = (short) (( wYPos - rectData.top ) / panAnalogOut.lRowHeight);
	return ( TextLineToChannelNumber ( nTextLine ) );
}
...
LONG ChannelToMouseCoords ( short nChannel )
{
	...
	**/
	...
	nTextLine = ChannelNumberToTextLine ( nChannel );
	return ( MAKELONG ( 0, rectData.top +
				   ( nTextLine * panAnalogOut.lRowHeight ) ) );
}
...
/**	InvalidateChannel	This function is passed a channel number
***				and a bit-packed flags value which indicates
***				which fields for that channel should be
***				invalidated ( and updated on the client ).
***				The function then calculates which parts of
***				the client are assigned to the channel and
***				invalidates as appropriate.
***	Parameters
***
***	 hwnd			Handle of client owner
***	 nChannel		Channel index ( see  conninfo.pciConnections )
***	 nChanFlags		May be a combination of the following
***				values :
***
***					CHANFIELD_NAME
***					CHANFIELD_BITMAP (exclaimation symbol)
***					CHANFILED_UNITS
***
***				NOTE CHANFIELD_VALUES is not required by this in
***				panel. To update the value editboxs call the
***				UpdateDataArea function which is responsible
***				for data area controls.
**/
VOID InvalidateChannel ( HWND hwnd, long lChannel, long lChanFlags )
{
	...
	lInvalidTextLine = ChannelNumberToTextLine ( (short) lChannel );

	rectInvalid.top    = rectData.top +
				( lInvalidTextLine * panAnalogOut.lRowHeight );

	rectInvalid.bottom = rectInvalid.top + panAnalogOut.lRowHeight;
	...
	**/
	...
	if ( ( lChanFlags & CHANFIELD_NAME ) > 0 ) {
	...
	if ( ( lChanFlags & CHANFIELD_BITMAP ) > 0 ) {

		rectInvalid.left   = min ( rectInvalid.left, rectSymbols.left );
		rectInvalid.right  = max ( rectInvalid.right,rectSymbols.right );

	}

	if ( ( lChanFlags & CHANFIELD_UNITS ) > 0 ) {
