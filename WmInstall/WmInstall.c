...
void RegisterDrivers ( char* cPath, char* cDir, char* cInf );
...
CHAR WINAPI WmDelete ( HWND hwnd, LPSTR szSrcDir, LPSTR szSupport,
					   LPSTR szInst, LPSTR szRes )
{
	...
	/** Rename any existing ini file to bak **/

	strcpy ( cImlini, szInstx );
	strcat ( cImlini, "\\iml.ini" );

	strcpy ( cImlbak, szInstx );
	strcat ( cImlbak, "\\iml.bak" );
	
	rename ( cImlini, cImlbak );
	

	/** Search the handlers list **/
	while ( bFound ) {
		
		/** Path to the local in file into cFilePath **/
		strcpy (cFilePath, szSupport);
		RemoveSlash ( cFilePath );
		...
		**/
		...
		nRet = GetPrivateProfileString( cHandler, "Exename", "",
									 cExeName, 12, cFilePath );

		/** If the file exists mark it as not archive **/
		MarkFile ( cExeName, szInstx );
	...
	while ( bFound ) {
		...
		/** Path to the local inf file into cFilePath **/
		strcpy (cFilePath, szSupport);
		RemoveSlash ( cFilePath );
		strcat (cFilePath, "\\iml.ini");		
		...
		**/
		...
		/** If the file exists mark it as archive **/
		MarkFile ( cExeName, szInstx );		
...

{
	...
	strcpy ( szSrcDirx, szSrcDir );
	RemoveSlash ( szSrcDirx ); 

	/** Initialise the path to iml.ini **/
	strcpy ( cImlini, szInstx );
	strcat ( cImlini, "\\iml.ini" );

	/** Initialise the path to iml.bak **/
	strcpy ( cImlbak, szInstx );
	strcat ( cImlbak, "\\iml.bak" );

	/** Add the apps to the software section of the ini **/
	CreateSoftwareList ( szInstx );

	/** Add the handlers to the hardware section of the ini.
	*** Also to the software section.
	**/
	CreateHardwareList ( szInstx );

	/** Move Copy control info to software list **/	
	AddDateToSoftwareList ();

	/** Find if any devices are already created.
	*** If so add them to the inf file.
	**/
	LookForDevices ( szSrcDirx, szInstx ) ;
	...
	/** Copy control **/
	InstallSoftwareFromINF ( cImlini );


	/** Register 64 and 32 bit drivers **/
	RegisterDrivers ( szInst, "Drivers64", "mlusb_01" );
	RegisterDrivers ( szInst, "Drivers32", "mlusb_02" );
...
/*********************************************
*** This routine finds if a driver directory is present
*** If its is register the Inf File
*** cPath will be the install dir ( with final slash )
*** cDir is directory name either Drivers32 or Drivers64
*** cInf if the inf file for that directory
**/
...

{
	...
	return;								/** Added to kill this function for now **/

	/** Create the path to the directory **/
	strcpy ( cFilePath, cPath );
	strcat ( cFilePath, "\\" );
	strcat ( cFilePath, cDir );

	if ( IsFilePresent ( cFilePath, cInf, ".inf" ) == 0 ){
		return;
	}

	/** Create the full path to the file **/
	strcat ( cFilePath, "\\" ); 
	strcat ( cFilePath, cInf );
	strcat ( cFilePath, ".inf" );


	if ( SetupCopyOEMInf ( cFilePath,NULL, SPOST_PATH, 0, NULL, 0, NULL, NULL )  == FALSE ) {
		dwError = GetLastError ();
		strcpy ( cTemp, "Failure    ");
		ultoa ( dwError, cTemp + 9, 10);

		MessageBox ( GetFocus (), "Error", cTemp, MB_OK );
	}
