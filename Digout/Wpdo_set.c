/**
***	WPDO_SET.c		ATK-PANELS
***				DigitalOut Application Setup File Support
***
***     Andrew Thomson
***
...
***
***	Return		See #defined Error codes (WPDO_DEF.h)
***
**/
short SaveAppSetup ( char *szAppSetup )
{
	...
	***
	**/
	strcpy ( panDigitalOut.szFileSpec, szAppSetup );
	...
	**/
	...
	/**	DDE Support
	**/
	sprintf ( szKey, "%u", (WORD) panDigitalOut.bDDEEnabled );
	...
	**/
	WritePrivateProfileString ( szSectionName, "DDEServiceName",
				  panDigitalOut.szDDEServiceName, szAppSetup );
...
short LoadAppSetup ( char *szAppSetup )
{
	...
	/**	Store the filename from wence we got the setup details
	***
	**/
	strcpy ( panDigitalOut.szFileSpec, szAppSetup );
	...
	**/
	...
	/**	DDE support
	**/
	panDigitalOut.bDDEEnabled = (BOOL) GetPrivateProfileInt (
						szSectionName,
						(LPCSTR) "DDEEnabled",
						0,
						(LPCSTR) szAppSetup );
	...
	**/
	...
	if ( conninfo.nNumConnected == 0 ) {
		...
		***	( although not directly used, these may be
		***	requested Over DDE )
		**/
		...
		**/
		...
		/**	Build a toolkit Monitor table, This states
		***	which IML device have support for alarms. It is
		*** 	required before attempting to read data.
		**/
	...
	**/
	...
	/**	Initialise cycling to read channels periodically
	***	This will detect channel outputs set internally
	***	in the hardware on alarms.
	***
	***	TO BE IMPLEMENTED
	***
	**/
